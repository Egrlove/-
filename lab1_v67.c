...
//===============================================
void assembling(double **finite_el, int  order, double *b_el, double u_beg, double u_end, double** res, double* b, int res_size)
{
    int step = order - 1;
...
void output( int length, int num_of_nodes, double* U, const char *file_name)
{
    ...
    {
        double y = C1 * exp(3. * x * sqrt(frac)) + C2 * exp(-3. * x * sqrt(frac)) + 10. / 3.;
...
/*    fclose(file);
    ...
    fprintf (gnu, "plot \"result.txt\" using 1:2 w li lw 2 lt rgb 'blue', \"result.txt\" using 1:3 w li lw 2 lt rgb 'red'\n");
...
void calculate( int length, int num_of_el, double** finite_el, int order, double* b_el, const char * file_name)
{
    ...
    for (int i = 0; i < order - 1; i++)
    {
        ...
        augmented_matrix[aug_n - 1 - i][aug_m - 1] -= U[num_of_nodes - 1] * finite_el[order - 2 - i][order - 1];
    }
